...
この手順書では、右手でハンダごてを操作する想定で記述を行っています。  
基板は、この画像の向きを標準とします。  
...
一度全てのダイオードの右側のみハンダ付け(4番まで)を行い、一気にテスターで向きを確認すると、効率的に作業できます。
...
基板下寄り(`R1`〜`R10`の直下)に横に並んでいる、 `D1`〜`D10` に、 表面実装ダイオード 1SS355VM をハンダ付けします。  
全てのダイオードは、白い線(カソード)が右側となるのが正しい向きになります。  
